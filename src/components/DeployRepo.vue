    ...
    <p col center>
      Username:
              ...
              <textarea
                ...
                v-model="env"
                :cols="24"
                @keyup.enter="change_vars()"
                placeholder="KEY=VALUE"
                @paste="change_vars()"
                @keyup.esc="payload.env_vars = []"
              ></textarea>

              <button class="btn-get" @click="trigger = true">Deploy</button>
              <Request
                :url="'/api/deploy/' + repository + '?user=' + state.user!.ref"
                :options="{
                  method: 'POST',
                  body: JSON.stringify({
                    port: payload.port,
              ...
              >
                <template #="{ json }">
                  <a :href="json.url" class="btn-get" target="_blank"
                    ><strong>Visit your brand new App!</strong></a
                  >
      </template>
      <template #loading>
