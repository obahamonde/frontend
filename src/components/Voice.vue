...
<template>
  <div v-if="speech.isSupported">
    <div v-if="voicesLength > 0 && toggle">
      <select class="input" v-model="currentVoice" @change="pickVoice(currentVoice)">
        <option  v-for="v in voices" :value="v">
          {{ v }}
        </option>
      </select>
      <p>Selected voice: <strong>{{ voice.name }}</strong></p>
    </div>
    <div v-if="isListening">
      <Icon v-if="isListening" class="btn-icon" icon="mdi-pause" @click="pause()" />
      <Icon v-if="isListening" class="btn-icon" icon="mdi-stop" @click="stop()" />
    </div>
    <div v-else>
