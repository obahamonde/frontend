...
<template>
  <section class="mt-12">
    <div v-if="prompt.length > 0 && thisPrompt.length === 0" class="mt-12 mx-8">
      <Request :url="'/api/completion?user=' + state.user!.ref!" :options="{
        method: 'POST',
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-gray-800 w-full px-4 py-4 row center">
      <img class="x3 rounded-full inline-block m-4 cp" :src="state.user!.picture" @click="showModal = true" />
      <input class="w-full px-4 py-2 outline-none rounded-md" type="text" v-model="thisPrompt"
        @keyup.enter="handlePrompt()" placeholder="Type something..." />
    </div>
  </section>
  <Modal v-if="showModal" @close="showModal = false">
    ...
    <template #body>
      <div class="flex flex-col items-center">
        <img class="x4 rounded-full inline-block mr-2" :src="state.user!.picture" />
