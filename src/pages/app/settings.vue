    ...
    <Request :url="'/api/codeserver?ref=' + state.user!.ref">
      <template #default="{ json }">
        <div class="col center p-12 gap-4 bg-gray-300 rounded sh">
          ...
          <p><strong>Created: </strong>{{ new Date(Number(json.ts)).toLocaleString() }}</p>
        </div>
      </template>
    </Request>
    <Request :url="'/api/db/' + state.user!.ref">
      <template #default="{ json }">
        ...
        <div class="col center p-12 gap-4 bg-gray-300 rounded sh">
          ...
          <p>
          <p class='row center gap-2'><strong>Database Secret Key: </strong>{{ show ? json.secret : '***********' }}
            <Icon icon="mdi-clipboard" @click="copy(json.secret)" class="text-primary hover:text-cyan cp scale" />
            <Icon icon="mdi-eye" class="text-primary hover:text-cyan cp scale" @click="show = !show" />
          </p>
          </p>
          <p><strong>Created: </strong>{{ new Date(Number(json.ts)).toLocaleString() }}</p>
        </div>
